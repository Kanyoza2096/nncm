// pages/scriptures.tsx
// ============================================================================
// NNCM Church Portal — Scripture Meditations
// Next.js static export with SEO, accessibility, and UX upgrades.
//
// NOTE: Scripture data is intentionally hardcoded — these are canonical Bible
// verses that don't change. The interactive features (themes, audio, meditation)
// are all client-side enhancements that don't affect SEO.
// ============================================================================
...
const CATEGORIES = ['All', 'Identity', 'Grace', 'Zoe Life', 'Faith', 'Victory'] as const
...
export default function ScripturesPage() {
  ...
  const filteredScriptures = SCRIPTURES.filter((scrip) => {
    ...
    const matchesSearch =
      scrip.text.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  const handlePrev = useCallback(() => {
    if (filteredScriptures.length === 0) return
    setCurrentIndex(
      (prev) => (prev - 1 + filteredScriptures.length) % filteredScriptures.length
    )
  }, [filteredScriptures])
  ...
  const handleSpeak = () => {
    ...
    const utterance = new SpeechSynthesisUtterance(
      `${currentScripture.text} - ${currentScripture.reference}`
    )
  ...
  return (
    <>
      ...
      {/* ================================================================== */}
      <Head>
        <title>Scripture Meditations — {churchName}</title>
        <meta
          name="description"
          content="Immerse yourself in beautiful animated scripture verses on identity, grace, faith, and victory. Designed to stir faith and awaken your new creation identity in Christ."
        />
        <meta
          name="keywords"
          content="scriptures, Bible verses, daily devotion, new creation, grace, faith, healing, NNCM, meditation"
        />

        {/* Open Graph */}
        <meta property="og:title" content={`Scripture Meditations — ${churchName}`} />
        <meta
          property="og:description"
          content="Animated scripture verses on identity, grace, and victory in Christ."
        />
        <meta property="og:type" content="website" />
        <meta property="og:url" content={`${siteUrl}/scriptures`} />
        <meta property="og:image" content={`${siteUrl}/logo.png`} />
        <meta property="og:site_name" content={churchName} />

        {/* Twitter */}
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:title" content={`Scripture Meditations — ${churchName}`} />
        <meta name="twitter:description" content="Animated scripture verses on identity, grace, and victory." />
        ...
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              ...
              '@type': 'WebPage',
              name: `Scripture Meditations — ${churchName}`,
              url: `${siteUrl}/scriptures`,
              description: 'Interactive scripture meditation experience.',
              about: {
                '@type': 'Church',
                name: churchName,
              },
      ...
      {/* ================================================================== */}
      <main
        className={`transition-all duration-1000 min-h-screen pt-24 pb-20 font-sans ${activeTheme.bgClass} flex flex-col justify-between`}
      >
            ...
            {!isMeditating && (
              ...
              >
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                  <div>
                    <div className="flex items-center gap-2 mb-1.5">
                      <span className="p-1 bg-indigo-500/10 rounded-lg text-indigo-400 border border-indigo-500/10 flex items-center gap-1 text-[10px] uppercase font-black tracking-widest">
                        <Sparkles className="w-3 h-3 text-indigo-400" aria-hidden="true" /> Wisdom Feed
                      </span>
                    </div>
                    ...
                    <p className="text-xs opacity-70 mt-1">
                      Behold Jesus. Walk in Zoe. Embrace your New Identity.
                    </p>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <div className="flex items-center gap-1.5 bg-slate-900/10 dark:bg-white/5 p-1 rounded-xl border border-slate-200/10">
                      <Layers className="w-3.5 h-3.5 opacity-60 ml-1.5" aria-hidden="true" />
                      {THEMES.map((theme) => (
                        <button
                          key={theme.id}
                          onClick={() => setActiveTheme(theme)}
                          className={`px-2.5 py-1 text-[10px] font-bold rounded-lg transition-all focus:outline-none focus:ring-2 focus:ring-indigo-400 ${
                    ...
                    <button
                      ...
                      className="p-2 bg-slate-900/10 dark:bg-white/5 hover:bg-slate-900/20 dark:hover:bg-white/10 border border-slate-200/10 rounded-xl transition focus:outline-none focus:ring-2 focus:ring-indigo-400"
                      aria-label={isSerif ? 'Switch to sans-serif font' : 'Switch to serif font'}
                    >
                    ...
                    <button
                      ...
                      className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-xl text-xs flex items-center gap-2 shadow-md transition-all active:scale-95 focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:ring-offset-2"
                    >
                      <Wind className="w-4 h-4 animate-pulse" aria-hidden="true" /> Meditate
                    </button>
                  </div>
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-4 justify-between items-stretch">
                  <div
                    className="flex flex-wrap gap-1.5 bg-slate-900/5 dark:bg-white/5 p-1 rounded-xl border border-slate-200/5 max-w-xl"
                    role="group"
                  ...
                  >
                    {CATEGORIES.map((cat) => (
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all focus:outline-none focus:ring-2 focus:ring-indigo-400 ${
                          currentCategory === cat
            ...
            <div className="w-full max-w-4xl relative min-h-[380px] flex flex-col justify-between">
              <AnimatePresence mode="wait">
                {filteredScriptures.length > 0 ? (
                  ...
                  >
                    <div className="absolute -top-24 -right-24 w-48 h-48 rounded-full bg-indigo-500/10 blur-3xl pointer-events-none" aria-hidden="true" />
                    <div className="absolute -bottom-24 -left-24 w-48 h-48 rounded-full bg-rose-500/5 blur-3xl pointer-events-none" aria-hidden="true" />

                    <div className="flex justify-between items-center mb-6">
                      <span
                        className={`px-2.5 py-1 text-[9px] uppercase font-black tracking-widest rounded-full ${activeTheme.accentClass}`}
                      >
                    ...
                    <div className="mt-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-t border-slate-200/10 pt-6">
                      <div className="flex items-center gap-2">
                        <BookOpen className="w-5 h-5 text-indigo-500 shrink-0" aria-hidden="true" />
                      ...
                      <div className="flex items-center gap-1.5 w-full sm:w-auto justify-end">
                        <button
                          ...
                          className="p-2.5 rounded-xl bg-slate-900/10 dark:bg-white/5 hover:bg-slate-900/20 dark:hover:bg-white/10 border border-slate-200/5 transition text-indigo-500 dark:text-indigo-400 active:scale-95 focus:outline-none focus:ring-2 focus:ring-indigo-400"
                        ...
                        <button
                          ...
                          className="p-2.5 rounded-xl bg-slate-900/10 dark:bg-white/5 hover:bg-slate-900/20 dark:hover:bg-white/10 border border-slate-200/5 transition active:scale-95 focus:outline-none focus:ring-2 focus:ring-indigo-400"
                        ...
                        <button
                          ...
                          className="p-2.5 rounded-xl bg-slate-900/10 dark:bg-white/5 hover:bg-slate-900/20 dark:hover:bg-white/10 border border-slate-200/5 transition active:scale-95 text-indigo-500 dark:text-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-400"
                          aria-label="Show random scripture"
                        >
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center text-center p-12 bg-slate-900/10 dark:bg-white/5 border border-slate-200/10 rounded-3xl min-h-[300px]">
                    <Compass className="w-12 h-12 text-slate-400 mb-4" aria-hidden="true" />
                    <h2 className="text-sm font-extrabold">No Scriptures Match Your Filter</h2>
                    <p className="text-xs opacity-60 mt-1">
                      Try resetting your category or clearing the search keyword.
                    </p>
                    <button
                      onClick={() => {
                      ...
                      className="mt-4 px-4 py-2 bg-indigo-600 text-white text-xs font-bold rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-400"
                    >
                      Reset Filters
                    </button>
                  </div>
              ...
              {filteredScriptures.length > 1 && (
                <>
                  <button
                    onClick={handlePrev}
                    className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-4 sm:-translate-x-12 p-3 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200 hover:text-indigo-600 border border-slate-200/10 rounded-full shadow-lg hover:scale-110 active:scale-90 transition-all z-10 focus:outline-none focus:ring-2 focus:ring-indigo-400"
                  ...
                  <button
                    onClick={handleNext}
                    className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-4 sm:translate-x-12 p-3 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200 hover:text-indigo-600 border border-slate-200/10 rounded-full shadow-lg hover:scale-110 active:scale-90 transition-all z-10 focus:outline-none focus:ring-2 focus:ring-indigo-400"
                ...
                >
                  ...
                  <div className="max-w-4xl mx-auto w-full flex-1 flex flex-col justify-between relative z-10">
                    <div className="flex justify-between items-center pb-6 border-b border-white/5">
                      <div className="flex items-center gap-2">
                        <Wind className="w-5 h-5 text-indigo-400 animate-pulse" aria-hidden="true" />
                        <div>
                      ...
                      <button
                        ...
                        className="px-4 py-2 border border-white/10 rounded-xl hover:bg-white/5 text-xs font-bold transition focus:outline-none focus:ring-2 focus:ring-white/30"
                      >
                        Exit Meditation
                      </button>
                    </div>
                    ...
                    <div className="my-auto space-y-12 py-8 flex flex-col items-center">
                      <div className="relative w-36 h-36 flex items-center justify-center mb-4">
                        ...
                        <motion.div
                          ...
                          aria-hidden="true"
                        />
                        <span className="relative text-xs font-black tracking-widest uppercase text-white">
                          {breathPhase}
                        </span>
                      </div>

                      <blockquote className="text-2xl sm:text-4xl text-center max-w-3xl leading-relaxed sm:leading-loose font-serif italic text-slate-100">
                        &ldquo;{currentScripture.text}&rdquo;
                      </blockquote>
                    ...
                    <div className="flex justify-between items-center border-t border-white/5 pt-6 text-slate-400">
                      <span className="text-[10px] uppercase font-bold tracking-wider">
                        Breathing rate: 4s-4s-4s
                      </span>
                      <div className="flex gap-4">
                        <button
                          onClick={handlePrev}
                          className="text-xs hover:text-white transition font-semibold focus:outline-none focus:ring-2 focus:ring-white/30 rounded px-2 py-1"
                        >
                          Prev Verse
                        </button>
                        <button
              ...
              >
                <div className="flex items-center gap-4">
                  <button
                    ...
                    className={`flex items-center gap-1 px-3 py-1.5 rounded-lg font-bold border border-slate-200/10 focus:outline-none focus:ring-2 focus:ring-indigo-400 ${
                      isPlaying
                  ...
                  >
                    {isPlaying ? (
                      <>
                        <Pause className="w-3.5 h-3.5 fill-current" aria-hidden="true" /> Auto-Cycle Active
                      </>
                    ) : (
                      <>
                        <Play className="w-3.5 h-3.5 fill-current" aria-hidden="true" /> Auto-Cycle Paused
                      </>
                    )}
                  </button>
                  ...
                  {isPlaying && (
                    <div className="flex items-center gap-2">
                      <label htmlFor="rotation-interval" className="opacity-70">
                        Interval:
                      </label>
                      <select
                        ...
                        className="bg-slate-900/10 dark:bg-white/5 border border-slate-200/10 rounded-lg p-1 text-[11px] font-bold focus:outline-none focus:ring-2 focus:ring-indigo-400"
                      >
                ...
                <div className="flex items-center gap-2 text-slate-400 text-[11px] font-medium">
                  <Award className="w-4 h-4 text-indigo-500" aria-hidden="true" />
                  <span>Inspired theology oversight by {churchName}</span>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
