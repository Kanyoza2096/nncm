...
export default function EventsCalendar() {
  ...
  const handleConnectCalendar = async () => {
    try {
      ...
      if (result) {
        ...
        toast.success('Connected to Google Calendar! Now click Sync below.');
      }
    } catch (err) {
  ...
  const handleSyncCalendarNow = async () => {
    ...
    try {
      const res = await syncChurchReminders(true); // default to Malawi CAT timezone
      ...
      toast.error('Sync failed', { description: err.message || 'Error creating recurring events.' });
    } finally {
            ...
            <div className="w-full lg:w-auto shrink-0 flex flex-col gap-3 min-w-[280px]">
              ...
              {!isGoogleLinked ? (
                <button
                  ...
                  className="w-full bg-white text-slate-900 hover:bg-slate-50 font-bold text-xs py-3.5 px-5 rounded-2xl shadow-md transition-all flex items-center justify-center gap-2 border border-slate-200 active:scale-95"
                >
                ...
                <button
                  ...
                  className="w-full bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-800/50 text-white font-bold text-xs py-3.5 px-5 rounded-2xl shadow-lg shadow-indigo-600/20 transition-all flex items-center justify-center gap-2 active:scale-95"
                >
                  <RefreshCw className={`w-4 h-4 ${isSyncingCalendar ? 'animate-spin' : ''}`} />
                  {isSyncingCalendar ? 'Synchronizing Google Calendar...' : 'Sync Weekly Services to Google Calendar'}
                </button>
              )}
