...
export const churchService = {
  ...
  sermons: {
    ...
    create: async (sermon: Omit<Sermon, 'id' | 'downloadsCount'>): Promise<string> => {
      ...
      if (shouldUseSupabase()) {
        try {
          id = await supabaseService.church.sermons.create(sermon);
        } catch (e) {
          console.error('[Supabase Bridge] Creating sermon failed:', e);
        }
      }
      if (!id) {
        id = 'serm-' + Math.random().toString(36).substring(2, 11);
      }
      const list = getList(KEYS.SERMONS, initialSermons);
    ...
    update: async (id: string, updates: Partial<Sermon>): Promise<void> => {
      if (shouldUseSupabase()) {
        try {
          await supabaseService.church.sermons.update(id, updates);
        } catch (e) {
          console.error('[Supabase Bridge] Updating sermon failed:', e);
        }
      }
      const list = getList(KEYS.SERMONS, initialSermons);
    ...
    delete: async (id: string): Promise<void> => {
      if (shouldUseSupabase()) {
        try {
          await supabaseService.church.sermons.delete(id);
        } catch (e) {
          console.error('[Supabase Bridge] Deleting sermon failed:', e);
        }
      }
      const list = getList(KEYS.SERMONS, initialSermons);
      const filtered = list.filter(s => s.id !== id);
    ...
    incrementDownload: async (id: string): Promise<void> => {
      if (shouldUseSupabase()) {
        try {
          await supabaseService.church.sermons.incrementDownload(id);
        } catch (e) {
  ...
  events: {
    ...
    create: async (evt: Omit<ChurchEvent, 'id' | 'registeredCount'>): Promise<string> => {
      ...
      if (shouldUseSupabase()) {
        try {
          id = await supabaseService.church.events.create(evt);
        } catch (e) {
    ...
    update: async (id: string, updates: Partial<ChurchEvent>): Promise<void> => {
      if (shouldUseSupabase()) {
        try {
          await supabaseService.church.events.update(id, updates);
        } catch (e) {
    ...
    delete: async (id: string): Promise<void> => {
      if (shouldUseSupabase()) {
        try {
          await supabaseService.church.events.delete(id);
        } catch (e) {
  ...
  devotionals: {
    ...
    create: async (dev: Omit<Devotional, 'id'>): Promise<string> => {
      ...
      if (shouldUseSupabase()) {
        try {
          id = await supabaseService.church.devotionals.create(dev);
        } catch (e) {
          console.warn('[Devotional Service] Supabase create failed:', e);
        }
      }
      if (!id) {
      ...
      filtered.unshift(newItem);
      saveList(KEYS.DEVOTIONALS, filtered);
      return id;
    }
  },
