...
export const projectService = {
  ...
  createProject: async (data: Omit<Project, 'id'>): Promise<string> => {
    ...
    if (shouldUseSupabase()) {
      try {
        id = await supabaseService.projects.createProject(data);
      } catch (e) {
        console.error('[Supabase Bridge] Project creation failed:', e);
      }
    }
    if (!id) {
  ...
  updateProject: async (id: string, data: Partial<Project>): Promise<void> => {
    if (shouldUseSupabase()) {
      try {
        await supabaseService.projects.updateProject(id, data);
      } catch (e) {
        console.error('[Supabase Bridge] Project update failed:', e);
      }
    }
    const list = getOfflineProjects();
  ...
  deleteProject: async (id: string): Promise<void> => {
    if (shouldUseSupabase()) {
      try {
        await supabaseService.projects.deleteProject(id);
      } catch (e) {
        console.error('[Supabase Bridge] Project deletion failed:', e);
      }
    }
    const list = getOfflineProjects();
    const filtered = list.filter(p => p.id !== id);
