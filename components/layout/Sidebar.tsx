...
import { 
  LayoutDashboard, Users, HeartHandshake, Briefcase, Receipt, UserCircle,
  Settings, LogOut, FileText, X, Activity, Shield, MessageSquare,
  Sparkles, BookOpen, BookMarked, Calendar, Image
} from 'lucide-react'
  ...
  return (
    <aside className="w-64 bg-white border-r border-slate-200 flex flex-col h-screen shrink-0">
      <div className="p-6 flex items-center justify-between">
        ...
        {onClose && (
          <button onClick={onClose} className="lg:hidden p-1 text-slate-400 hover:text-slate-900" aria-label="Close sidebar">
            <X className="w-5 h-5" />
          </button>
        )}
      </div>
      ...
      <nav className="flex-1 px-4 mt-2 space-y-1.5 overflow-y-auto">
        ...
        {isAdmin && (
          ...
          >
            <Settings className={`mr-3 h-4 w-4 ${location.startsWith('/admin/settings') ? 'text-white' : 'text-slate-400'}`} />
            Settings
          </Link>
        )}
      </nav>
      ...
      <div className="p-4 mt-auto border-t border-slate-200">
        <div className="flex items-center gap-3 mb-4 px-2">
          <div className="w-10 h-10 rounded-full bg-indigo-100 p-1 border border-indigo-200 flex items-center justify-center text-indigo-600 font-bold shrink-0">
            {profile?.name?.charAt(0) || 'U'}
          </div>
          <div className="flex-1 overflow-hidden">
            <p className="text-sm font-semibold truncate text-slate-900">{profile?.name || 'User'}</p>
            <p className="text-xs text-slate-500 truncate capitalize">{profile?.role?.replace('_', ' ') || 'Guest'}</p>
          </div>
        </div>
        <button
          onClick={() => logout()}
          className="flex items-center w-full px-4 py-2 text-sm font-medium text-slate-600 opacity-70 hover:opacity-100 hover:bg-slate-100 rounded-xl transition-colors cursor-pointer"
        >
          <LogOut className="mr-3 h-5 w-5 text-red-600" strokeWidth={1.5} />
          Logout
        </button>
      </div>
    </aside>
